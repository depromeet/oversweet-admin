...
//        String[] arr = {"캐모마일", "티", "얼그레이", "아이스티", "루이보스"};
        ...
        for (Row row : drinkResponse.getI2790().getRow()) {
            if (drinkEntityRepository.findByName(row.getDescKor()).isPresent()) {
                continue;
            }
            final var drink = Drink.builder()
                    .name(row.getDescKor())
                    .size(Integer.parseInt(row.getSize()))
                    .sugar(Integer.parseInt(row.getSugars()))
                    .calorie(Integer.parseInt(row.getKcal()))
                    .imageUrl(null)
